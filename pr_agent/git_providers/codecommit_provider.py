...
class CodeCommitProvider(GitProvider):
    ...
    def _get_language_percentages(extensions):
        ...
        # Optimized: Use a single pass to count and calculate percentages with a dictionary,
        # avoiding the temporary Counter object and looping overhead.
        percentage = {}
        for ext in extensions:
            percentage[ext] = percentage.get(ext, 0) + 1
        # In-place update to percentage for final result (save an extra dictionary creation)
        for ext in percentage:
            percentage[ext] = round(percentage[ext] / total_files * 100)
        return percentage
